...
#     * Phase 3 で追加された OGP 画像の自サーバ配信パス
#       (例: <img src="/ogp-images/<sha1>.<ext>">) は http(s):// で始まらない
#       ため、このスクリプトの検知対象外で自動的に素通る。
#     * nginx 側の CSP `img-src 'self'` により外部ホストからの <img> 読み込み
#       は block される想定 (Phase 3 Batch D)。
