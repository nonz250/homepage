 ...
 *
 * `articles/` (Zenn 共有) と `site-articles/` (本サイト限定) の 2 ディレクトリを
 * 同一コレクションとして読み込む構成では、同じ slug (= ファイル名) のファイルが
 * 両方に存在するとどちらが優先されるか不定になる。build 前にこの状態を検出し、
 * 明示的なエラーで停止させるためのロジックを副作用なしで提供する。
 *
